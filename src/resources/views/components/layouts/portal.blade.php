            ...
            <div class="flex h-full min-h-0 flex-col">
                ...
                <nav class="flex-1 min-h-0 overflow-y-auto px-5 py-6 space-y-2">
                    ...
                    @elseif(($portalRole ?? null) === 'admin')
                        ...
                        @endcan
                        @if(auth()->user() && collect(['privacy.retention.view', 'privacy.holds.view', 'privacy.disposition.plan', 'privacy.disposition.authorize', 'privacy.disposition.execute'])->contains(fn ($permission) => auth()->user()->hasDirectPermission($permission)))
                            <a href="{{ route('admin.retention.index') }}" class="flex items-center gap-3 rounded-xl px-4 py-3 font-medium transition-all duration-200 {{ request()->routeIs('admin.retention.*') ? 'bg-white/20 border border-white/15 shadow-sm' : 'hover:bg-white/10 hover:translate-x-1' }}">
