@if($jobs->isEmpty())
    <div class="rounded-3xl bg-white p-8 shadow border border-gray-100 text-center">
        ...
        <p class="mt-2 text-gray-500">
            {{ ($status || $listingType) ? 'No listings match your current filters.' : 'Create your first job listing to begin receiving applicants.' }}
        </p>
    </div>
@else
    <div class="space-y-3">
            ...
            <x-likeslocale.operation-row>
                <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5">
                    <a href="{{ route('employer.jobs.edit', $job) }}" class="flex items-start gap-4 min-w-0 flex-1">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <h3 class="text-lg font-semibold tracking-[0.02em] text-[#6f4cb2]">{{ $job->title }}</h3>
                                <x-likeslocale.status-pill tone="brand">{{ \App\Models\Job::listingTypeLabelFor($job->listing_type) }}</x-likeslocale.status-pill>
                                <x-likeslocale.status-pill :tone="\App\Models\Job::toneFor($job->status)">{{ \App\Models\Job::labelFor($job->status) }}</x-likeslocale.status-pill>
                    ...
                    <div class="xl:shrink-0">
                        <x-likeslocale.button :href="route('employer.jobs.edit', $job)" variant="info">Edit</x-likeslocale.button>
                    </div>
                </div>
            </x-likeslocale.operation-row>
        @endforeach
